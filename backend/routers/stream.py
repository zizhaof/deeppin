...
async def chat(thread_id: uuid.UUID, body: ChatRequest):
    """
    ...
    # 检查线程是否存在；使用 maybe_single() 避免0条记录时 SDK 抛异常
    # Check thread existence; use maybe_single() so missing records return None instead of raising
    ...
    return StreamingResponse(
        ...
        headers={
            ...
            "X-Accel-Buffering": "no",  # 禁用 Nginx 缓冲，SSE 必须 / Disable Nginx buffering (required for SSE)
        },
    )
