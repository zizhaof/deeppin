...
用户浏览器
    ...
    │
    └── HTTPS → Oracle Free Tier（后端 FastAPI + 监控栈）
                域名：deeppin.duckdns.org
                Nginx 容器 反向代理 → backend:8000（compose 网络）
                LiteLLM Router 内嵌在 backend 进程中
                监控：Prometheus（scrape backend /metrics）+ Grafana（/grafana/ 子路径）

Oracle Free Tier 配置：
  - 4核 ARM，24GB 内存（永久免费）
  - Ubuntu 22.04
  - Docker Compose 管理服务（prod + staging 同机）
```

**docker-compose.yml 关键服务**（完整见仓库根）：
- `backend`：FastAPI + LiteLLM Router + SentenceTransformer，挂载共享 `hf-cache` volume 存 bge-m3
- `searxng`：自托管搜索
- `nginx`：TLS 终止 + 反代 backend / grafana，屏蔽公网访问 `/metrics`
- `prometheus`：`127.0.0.1:9090` 仅回环，90d / 10GB 保留，scrape `backend:8000/metrics`
- `grafana`：`GF_SERVER_SERVE_FROM_SUB_PATH=true`，通过 nginx `/grafana/` 路径对外

**共享 named volume**：
- `deeppin-hf-cache` (external)：prod + staging backend 共用一份 bge-m3 模型（~4.3G），省 50% 磁盘
  - 定义见 docker-compose.yml `volumes` 段的 `external: true`，需在 Oracle 上先 `docker volume create deeppin-hf-cache`

**nginx**（容器内，`nginx/deeppin.conf`）：
- `/` → `backend:8000`（compose 网络，SSE buffering off / 5min timeout）
- `/grafana/` → `grafana:3000`（proxy_pass **不带** trailing slash，保留 `/grafana/` 前缀）
- `location = /metrics { return 404 }` 挡外网，仅 compose 内 prometheus 能拉
- staging：`staging-deeppin.duckdns.org` → `host.docker.internal:8001`
...
# compose.env（Oracle 生产，docker compose --env-file 读取）
BACKEND_PORT=8000
SEARXNG_PORT=8081
HTTP_PORT=80
HTTPS_PORT=443
SEARXNG_INTERNAL=http://searxng:8080
GRAFANA_ADMIN_PASSWORD=xxx              # Grafana 首次 boot seed 密码；UI 改完以 grafana_data volume 为准
...
| Grafana | https://deeppin.duckdns.org/grafana/（admin 密码在 compose.env 的 `GRAFANA_ADMIN_PASSWORD`，首次登录后可在 UI 改，改完持久化到 `grafana_data` volume）|
| Prometheus | `127.0.0.1:9090` 仅回环，通过 `ssh -L 9090:127.0.0.1:9090 oracle` 本机打隧道访问 |
| `/metrics` | 公网 404（nginx 屏蔽），只有 compose 内 `prometheus` 能拉 `backend:8000/metrics` |
