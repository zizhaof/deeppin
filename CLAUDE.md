...
- **searxng 容器 chown 配置目录**：searxng 容器启动时把 mount 进去的 `/etc/searxng/`（host 上是 `searxng/`）chown 到自己内部 UID 977。host 上的 ubuntu 用户因此失去对**目录**的写权限，git checkout/reset 想 unlink 里面的 `settings.yml` 时报 `Permission denied`。deploy workflow 已经在每次 git 之前 `sudo chown -R ubuntu:ubuntu searxng/` 兜底。手动跑命令时（比如 `git pull` 看现状）撞到了，自己 chown 一下就行。
