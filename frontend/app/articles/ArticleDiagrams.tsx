...
export function SystemConnectivityDiagram() {
  ...
  return (
    <SvgWrap w={W} h={H}>
      ...
      <text x={c2} y={headerY} textAnchor="middle" fill={C.accent} fontSize={9} fontWeight={700} fontFamily="inherit" letterSpacing="0.08em">ROUTERS</text>
      ...
      <Box x={col3x} y={r2} w={col3w} h={BH} label="context_builder" sub="compact + ancestor" />
      <Box x={col3x} y={r3} w={col3w} h={BH} label="memory_service" sub="dual-track RAG" />
      <Box x={col3x} y={r4} w={col3w} h={BH} label="search_service" sub="SearXNG wrapper" />
      <Box x={col3x} y={r5} w={col3w} h={BH} label="llm_client" sub="SmartRouter" warn />
      <Box x={col3x} y={r6} w={col3w} h={BH} label="attachment_proc" sub="extract + chunk" />
      <Box x={col3x} y={r7} w={col3w} h={BH} label="embedding_service" sub="encode + vectorize" />

      {/* ─── ROUTERS (col2) ───────────────────────────────────────── */}
      <Box x={col2x} y={r1} w={col2w} h={BH} label="stream.py" sub="POST /threads/:id/chat" accent />
      <Box x={col2x} y={r2} w={col2w} h={BH} label="threads.py" sub="CRUD + suggestions" />
      <Box x={col2x} y={r3} w={col2w} h={BH} label="sessions.py" sub="CRUD + bulk messages" />
      <Box x={col2x} y={r4} w={col2w} h={BH} label="search.py" sub="SSE web search" />
      <Box x={col2x} y={r5} w={col2w} h={BH} label="merge.py" sub="structured output" />
      <Box x={col2x} y={r6} w={col2w} h={BH} label="attachments.py" sub="file upload" />
      <Box x={col2x} y={r7} w={col2w} h={BH} label="health.py" sub="/health aggregation" />
      <Box x={col2x} y={r8} w={col2w} h={BH} label="relevance.py" sub="thread relevance" />

      {/* ─── FRONTEND (col1) ──────────────────────────────────────── */}
      <Box x={col1x} y={r1} w={col1w} h={BH} label="InputBar" sub="text + file + voice" accent />
      <Box x={col1x} y={r2} w={col1w} h={BH} label="MessageBubble" sub="render + pin select" />
      <Box x={col1x} y={r3} w={col1w} h={BH} label="PinMenu" sub="floating toolbar" />
      <Box x={col1x} y={r4} w={col1w} h={BH} label="MergeOutput" sub="merge panel" />
      <Box x={col1x} y={r5} w={col1w} h={BH} label="ThreadNav" sub="breadcrumb + tree" />
      <Box x={col1x} y={r6} w={col1w} h={BH} label="sse.ts" sub="SSE client" />
      <Box x={col1x} y={r7} w={col1w} h={BH} label="Zustand Stores" sub="thread + lang + theme" />
      <Box x={col1x} y={r8} w={col1w} h={BH} label="SessionDrawer" sub="history list" />
      ...
      {/* Docker label */}
      <rect x={col2x-10} y={H-28} width={col3x+col3w-col2x+20} height={18} rx={4}
        fill="none" stroke={C.textMut} strokeWidth={0.5} strokeDasharray="3 2" />
      <text x={(col2x+col3x+col3w)/2} y={H-16} textAnchor="middle" fill={C.textMut} fontSize={7.5} fontFamily="inherit">
        Docker Compose · Oracle Cloud Free Tier (4-core ARM · 24GB)
      </text>

      {/* Vercel label */}
      <rect x={col1x-5} y={H-28} width={col1w+10} height={18} rx={4}
        fill="none" stroke={C.textMut} strokeWidth={0.5} strokeDasharray="3 2" />
      <text x={c1} y={H-16} textAnchor="middle" fill={C.textMut} fontSize={7.5} fontFamily="inherit">
        Vercel · Next.js
      </text>
    </SvgWrap>
  );
...
export const DIAGRAMS: Record<string, React.ComponentType> = {
  ...
  "system-connectivity": SystemConnectivityDiagram,
};
