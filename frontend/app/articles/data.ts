// All article content (bilingual: Chinese and English).
...
export const articles: Article[] = [
      ...
      zh: {
        ...
        body: [
          ...
          { type: "ul", items: [
            ...
            "OpenRouter — 4 个 :free 模型（nemotron-super-120b、gpt-oss-120b、llama-3.3-70b、qwen3-next-80b），作为兜底 slot",
          ]},
  ...
  // ─────────────────────────────────────────────────────────────
  // Core innovation: Pin context system
  // ─────────────────────────────────────────────────────────────
    ...
    content: {
      zh: {
        ...
        body: [
          ...
          { type: "p", text: "SmartRouter 叠加了 5 家免费 Provider（Groq、Cerebras、SambaNova、Gemini、OpenRouter），生产配置 13 keys、共 43 slots（2 Groq×6 + 3 Cerebras×2 + 3 SambaNova×3 + 2 Gemini×2 + 3 OpenRouter×4）：" },
          { type: "ul", items: [
            "全局峰值 TPM：~2.52M（≈ 42,000 tok/s sustained，相当于 ~1.5 台 8×H100 DGX 满载 / 约 12 张 H100）",
            "全局每日 RPD 上限：约 128,000 次请求（五家累加）",
            "全局每日 token 额度：~236M tokens/day",
            "按每条消息 ~2.5K tokens 算，可支撑约 94,000 条消息/天",
            "每日可完整对话：9,400-18,800 个（假设每对话 5-10 轮）",
            "折算 DAU：约 9,000-15,000 个活跃用户（假设每人每天 6 轮对话）",
          ]},
          { type: "p", text: "瓶颈维度是 chat 组的 RPD——全局 56.5K/天，其中 Cerebras 两个 235B MoE slot 贡献 77%。OpenRouter 单 key 仅 50 RPD/模型（4 模型 ×50 = 200 RPD/key），SambaNova 单模型每 key 只有 20 RPD，Gemini 2.5 Flash 每 key 250 RPD、Flash-Lite 1K RPD。" },
          { type: "note", text: "优化路径：(1) OpenRouter 充 $10 credit——4 模型 × 3 key 的 RPD 从 50 升到 1000（20× 提升，一次性成本），是当前 ROI 最高的一步；(2) 加 Cerebras 第 4 个 key 线性扩 chat RPD（这条链子最紧的地方）；(3) 给 Groq 再加一 key 补 summarizer/vision 带宽；最终形态启用本地推理（Ollama + 量化）做极端兜底。" },
          ...
          { type: "code", text: `组件                  免费上限                  当前利用率    瓶颈?\nLLM API (5 Provider)  ~9000-15000 DAU           低            ★★  chat RPD 紧\nSupabase DB           ~7500 注册用户            低            ★★★ 存储紧\nOracle ARM (CPU)      ~2-3个并发上传            极低          ★   上传时\nOracle ARM (mem)      ~6000 并发连接            极低          ·   过剩\nVercel                ~100K 页面加载/月         极低          ·   过剩` },
          { type: "p", text: "当前阶段（早期用户验证），这套系统完全够用。SmartRouter 将五家 Provider 的额度叠加后，LLM 瓶颈从单 Provider 的 300 DAU 提升到了 9000+ DAU——已经不再是最紧的环节，反而 Supabase 500MB 存储成了下一个上限（约 7500 注册用户）。" },

          { type: "h1", text: "七、瓶颈突破优先级" },
          { type: "ul", items: [
            "第 1 优先：OpenRouter 充 $10 credit——12 个 slot 的 RPD 从 50 升到 1000（20× 提升，一次性成本，永久生效）",
            "第 2 优先：Supabase 存储到 400MB 时升级到 $25/月 plan，获得 8GB 存储",
            "第 3 优先：再加 1 个 Cerebras key 扩 chat RPD（Cerebras 贡献 77% chat RPD，线性扩展，零成本）",
      ...
      en: {
        ...
        body: [
          ...
          { type: "p", text: "SmartRouter stacks 5 free providers (Groq, Cerebras, SambaNova, Gemini, OpenRouter). Production config: 13 keys, 43 slots total (2 Groq×6 + 3 Cerebras×2 + 3 SambaNova×3 + 2 Gemini×2 + 3 OpenRouter×4):" },
          { type: "ul", items: [
          ...
          { type: "p", text: "The bottleneck is chat-group RPD — ~56.5K/day total, of which Cerebras's two 235B MoE slots contribute 77%. OpenRouter is 50 RPD per model per key (4 models × 50 = 200 RPD/key), SambaNova is 20 RPD per model per key, Gemini 2.5 Flash is 250 RPD per key, Flash-Lite is 1K RPD." },
          { type: "note", text: "Optimization path: (1) add $10 credit on OpenRouter — the 12 slots jump from 50 to 1000 RPD each (20×, one-time cost, permanent), highest-ROI move right now; (2) add a 4th Cerebras key to linearly expand chat RPD (the tightest link); (3) add another Groq key to back up summarizer / vision bandwidth. End state: enable local inference (Ollama + quantized) as extreme fallback." },
          ...
          { type: "p", text: "At the current stage (early user validation), this stack is entirely sufficient. SmartRouter's five-provider stacking raised the LLM bottleneck from 300 DAU (single provider) to 9,000+ DAU — LLM is no longer the tightest link. Supabase's 500MB storage cap (~7,500 registered users) is now the next bound." },
    ...
    content: {
      zh: {
        ...
        body: [
          ...
          { type: "p", text: "先看数字。17 个模型按生产配置（2 Groq×6 + 3 Cerebras×2 + 3 SambaNova×3 + 2 Gemini×2 + 3 OpenRouter×4 = 13 keys，共 43 slots）全量跑时：" },
          { type: "ul", items: [
            "峰值吞吐 ~2.52M TPM ≈ 42,000 tokens/sec sustained",
            "日额度 ~236M tokens/day，按每条消息 2.5K tokens 算，可支撑约 94K 条/天",
            "最大可调用模型规模：671B MoE / 37B 活跃（SambaNova 的 DeepSeek-V3.2）、400B MoE / 17B 活跃（SambaNova 的 Llama-4-Maverick）",
          ]},
          { type: "p", text: "换算成自托管硬件：42K tok/s sustained 吞吐相当于 1.5 台满载的 8×H100 DGX（约 12 张 H100 饱和，硬件 ~$600K + 机房 / 电 / 冷却 / 运维 ~$15K/月），按 3 年折旧每月约 $30K。Deeppin 的实际月账单：$0——因为 5 家 provider 的免费 tier 叠加起来刚好覆盖这个量级。" },
          { type: "p", text: "模型多样性另算。本池子最大的是 671B MoE（DeepSeek-V3.2）和 400B MoE（Llama-4-Maverick）；如果自己跑这个级别的模型，光装权重就要 16+ 张 H100 FP8（$400K+ 硬件就只为这一个模型），租是 $50-100/小时。这里是「免费 + 按需」。" },
          { type: "note", text: "42K tok/s 是所有 slot 同时跑满的理论峰值。实际日内用量分布不均，单次并发能打到 5-10K tok/s，已经远超单人/小团队的真实需求。Deeppin 当前日活个位数，算力利用率不到 1%。" },
          ...
          { type: "code", text: "# 例：Groq 从 2 key 扩到 3 key\n# backend/.env\nGROQ_API_KEYS=[\"gsk_key1\",\"gsk_key2\",\"gsk_key3\"]\n\n# SmartRouter 启动时看到 3 个 key × 6 个模型 = 18 个 slot\n# RPM/TPM/RPD/TPD 每维度线性扩 50%（原 2 key → 12 slot）" },
          ...
          { type: "code", text: "#  Provider/Model                                  多模态特点              RPM  TPM   RPD   TPD\n────────────────────────────────────────────────────────────────────────────────────────────\n1  gemini/gemini-2.5-flash                         Google 原生多模态，最强 10   250K  250   50M\n2  groq/meta-llama/llama-4-scout-17b-16e           Llama 4 多模态，速度快  30   30K   1K    500K" },

          { type: "note", text: "关键互补：Gemini 双 key 共 500K TPM、200M TPD（占全局 TPD 85%）是 merge 主力；SambaNova 三个 100K TPM 模型（含 DeepSeek-V3.2）撑住 merge 备份和 70B/400B chat；Cerebras 跑 235B MoE 拿到「大模型 + 60K TPM + 极快速度」三合一，同时 3 key 的 14.4K RPD/模型 是 chat RPD 主力（贡献 77%）；OpenRouter 包含 120B 推理模型（nvidia/nemotron-super）和长上下文 qwen3-next-80b；Groq 速度快、summarizer RPD 最高（14.4K）但单模型 TPM 最小，适合高频小请求。" },
          ...
          { type: "p", text: "GET /health/providers/keys 逐个验证每个 (provider, key) 的 /v1/models 校验和模型 catalog 漂移，不消耗 LLM quota（daily CI 跑这个端点）；GET /health/providers/full 则对每个 slot 真跑一次推理（会消耗 quota，只在手动深度巡检时用）：" },
          { type: "code", text: "GET /health/providers/keys\n{\n  \"total\": 13,\n  \"ok\": 12,\n  \"failed\": 1,\n  \"results\": [\n    {\"provider\": \"groq\", \"key\": \"gsk_abc1...\", \"ok\": true, \"models_seen\": 6},\n    {\"provider\": \"cerebras\", \"key\": \"csk_xyz...\", \"ok\": true, \"models_seen\": 2},\n    {\"provider\": \"sambanova\", \"key\": \"sk_...\", \"ok\": false, \"error\": \"401 Unauthorized\"},\n    ...\n  ]\n}" },

          { type: "h1", text: "十二、实际容量估算" },
          { type: "p", text: "以当前生产配置为例：2 Groq + 3 Cerebras + 3 SambaNova + 2 Gemini + 3 OpenRouter = 13 keys，共 43 slots（按 provider 的模型数加权：2×6 + 3×2 + 3×3 + 2×2 + 3×4）。" },
          { type: "ul", items: [
            "chat 组：38 slots，累计 ~920 RPM、~2.33M TPM、~56.5K RPD、~232M TPD",
            "merge 组：18 slots，累计 ~410 RPM、~1.66M TPM、~47.9K RPD、~106M TPD",
            "summarizer 组：7 slots，累计 ~180 RPM、~692K TPM、~74K RPD、~104M TPD",
            "vision 组：4 slots，累计 ~80 RPM、~560K TPM、~2.5K RPD、~101M TPD",
            "全局峰值 TPM：~2.52M（≈ 42,000 tok/s sustained，相当于 ~1.5 台 8×H100 DGX / 约 12 张 H100 满载）",
            "全局日额度：~236M tokens/day，按每条消息 2.5K tokens 算，可支撑 ~94K 条/天",
            "折算 DAU：约 9,000-15,000 活跃用户（每人每天 6 轮对话）",
          ]},
          { type: "p", text: "相比单 Provider Groq 的 300-600 DAU，五家叠加将容量提升约 25 倍。瓶颈在 chat 组 RPD（56.5K/天，其中 Cerebras 两个 slot 贡献 77%）——OpenRouter 单 key 仅 50 RPD/模型（4 模型 ×50 = 200 RPD/key），SambaNova 单模型每 key 只有 20 RPD，Gemini 2.5 Flash 250 RPD。继续扩容的最高 ROI 是 OpenRouter 充 $10 credit（12 slot 的 RPD 从 50 升到 1000，一次性成本换永久 20× 提升）。" },
        ],
      },
      en: {
        ...
        body: [
          ...
          { type: "p", text: "The numbers first. At the current production setup (2 Groq×6 + 3 Cerebras×2 + 3 SambaNova×3 + 2 Gemini×2 + 3 OpenRouter×4 = 13 keys across 43 slots), running full tilt:" },
          { type: "ul", items: [
          ...
          { type: "p", text: "Hardware equivalent: 42K tok/s sustained is roughly 1.5× what a fully loaded 8×H100 DGX delivers — ~12 saturated H100s, or ~$600K hardware + ~$15K/month in colo/power/cooling/ops, or ~$30K/month amortized over 3 years. Deeppin's actual monthly bill: $0. Five free tiers stacked happens to cover exactly this throughput band." },
          { type: "p", text: "Model diversity is a separate story. The largest models in the pool are 671B MoE (DeepSeek-V3.2) and 400B MoE (Llama-4-Maverick). Self-hosting a model at that scale needs 16+ H100s just to load weights at FP8 ($400K+ of hardware for that single model), or rents at $50-100/hour. Here it's free and on-demand." },
          { type: "note", text: "42K tok/s is the theoretical peak with every slot maxed simultaneously. Real-world load is uneven — a single concurrent burst hits 5-10K tok/s, still far beyond what a solo dev or small team needs. Deeppin's current DAU is single-digit; headroom utilization is under 1%." },
          ...
          { type: "p", text: "GET /health/providers/keys validates each (provider, key) against /v1/models and detects catalog drift without burning any LLM quota — the daily CI runs this endpoint. GET /health/providers/full actually fires a completion against every slot (consumes quota; used only for manual deep checks):" },
          { type: "code", text: "GET /health/providers/keys\n{\n  \"total\": 13,\n  \"ok\": 12,\n  \"failed\": 1,\n  \"results\": [\n    {\"provider\": \"groq\", \"key\": \"gsk_abc1...\", \"ok\": true, \"models_seen\": 6},\n    {\"provider\": \"cerebras\", \"key\": \"csk_xyz...\", \"ok\": true, \"models_seen\": 2},\n    {\"provider\": \"sambanova\", \"key\": \"sk_...\", \"ok\": false, \"error\": \"401 Unauthorized\"},\n    ...\n  ]\n}" },

          { type: "h1", text: "Part 12 — Actual capacity estimate" },
          { type: "p", text: "Current production config: 2 Groq + 3 Cerebras + 3 SambaNova + 2 Gemini + 3 OpenRouter = 13 keys, 43 slots total (weighted by per-provider model count: 2×6 + 3×2 + 3×3 + 2×2 + 3×4)." },
          { type: "ul", items: [
          ...
          { type: "p", text: "Compared to the original Groq-only setup (300-600 DAU), five-provider stacking lifts capacity by ~25x. The bottleneck is chat-group RPD (56.5K/day; Cerebras's two slots contribute 77%) — OpenRouter's free tier is only 50 RPD per model per key (4 models × 50 = 200 RPD/key), SambaNova is 20 RPD per model per key, Gemini 2.5 Flash is 250 RPD per key. Highest-ROI next step: add $10 credit on OpenRouter (the 12 slots jump from 50 to 1000 RPD — one-time cost for a permanent 20× boost)." },
        ],
      },
    },
  },
